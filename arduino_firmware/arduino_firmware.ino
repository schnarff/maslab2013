...
void getGyroValues(){
  ...
  int gyro_x = ((xMSB << 8) | xLSB);

  byte yMSB = readRegister(L3G4200D_Address, 0x2B);
  byte yLSB = readRegister(L3G4200D_Address, 0x2A);
  int gyro_y = ((yMSB << 8) | yLSB);

  byte zMSB = readRegister(L3G4200D_Address, 0x2D);
  byte zLSB = readRegister(L3G4200D_Address, 0x2C);
  int gyro_z = ((zMSB << 8) | zLSB);
  
  float yea = 1.2f/(70.0*10*4*1.4);
  gyro_ang += gyro_z*yea;
}
  ...
  */
    ...
    if (numImus > 0)
    {
      ...
      */
      ...
      int headingDegrees = ((int)(gyro_ang))%360;
